...
local VIEWER_VERSION = (C_AddOns and C_AddOns.GetAddOnMetadata and C_AddOns.GetAddOnMetadata(addonName, "Version")) or "?"

local verPending
local function SendVersion(delay)
    if verPending then verPending:Cancel() end
    verPending = C_Timer.NewTimer(delay or 1, function()
        verPending = nil
        local ch, tgt
        ...
        C_ChatInfo.SendAddonMessage(VER_PREFIX, "VER\t" .. VIEWER_VERSION, ch, tgt)
    end)
end
...
SlerneNotesViewer.frame:SetScript("OnEvent", function(self, event, ...)
    ...
    elseif event == "GROUP_ROSTER_UPDATE" then
        UpdateRoster()
        SendVersion(2 + math.random() * 2)
    elseif event == "CHAT_MSG_ADDON" then
        local prefix, text, channel, sender = ...
        if prefix == VER_PREFIX then
            if text == "REQ" then SendVersion(0.2 + math.random() * 1.5) end
        elseif prefix == "SlerneNotes" then
            ...
            if isComplete then
                local fullStr = table.concat(chunks[msgID], "", 1, totalChunks)
                chunks[msgID] = nil
                chunkTime[msgID] = nil
