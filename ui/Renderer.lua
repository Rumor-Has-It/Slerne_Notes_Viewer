...
local function GetImagePath(imgName)
    ...
    -- Subfolder path -> maps/base (season\raid\Fight.tga); bare filename ->
    -- maps/custom (manual entry / legacy data). Matches SlerneNotes.
...
-- Widest single (newline-delimited) line of text, in pixels -- sizes Text Blocks.
local measureFS
...
-- Helper to get the local player's short name
local function GetMyName()
    local name = UnitName("player")
    if name then
        -- Strip server name if it exists to match standard assignment formats
...
-- =======================
-- DRAWINGS OVERLAY (received lines + markers; view-only)
-- =======================
...
local lineObjPool = {}  -- placeable line/arrow objects (read-only here): {main,head1,head2}
...
local function arrowBarbs(x1, y1, x2, y2, size)
    ...
    return x2 + size * (rx * ca - ry * sa), y2 + size * (rx * sa + ry * ca),
           x2 + size * (rx * ca + ry * sa), y2 + size * (-rx * sa + ry * ca)
end
...
function SlerneNotesViewer.RenderDrawings(drawings)
    -- Render() Clear()s every child of canvasPanel (incl. this layer) -> re-show.
    drawLayer:Show()
    ...
    for _, o in ipairs(lineObjPool) do o.main:Hide(); o.head1:Hide(); o.head2:Hide() end
    ...
    for idx, mk in ipairs(drawings.markers or {}) do
        ...
        elseif mk.kind == "bossicon" then
            -- Boss / add icon: path relative to img/maps/base (same as the host).
    ...
    -- placeable line / arrow objects (read-only)
    for idx, ln in ipairs(drawings.lines or {}) do
        local o = lineObjPool[idx]
        if not o then
            o = { main = drawLayer:CreateLine(nil, "ARTWORK"),
                  head1 = drawLayer:CreateLine(nil, "ARTWORK"),
                  head2 = drawLayer:CreateLine(nil, "ARTWORK") }
            lineObjPool[idx] = o
        end
        ...
        o.main:Show()
        if ln.arrow then
            local b1x, b1y, b2x, b2y = arrowBarbs(x1, y1, x2, y2, 10 + th * 1.4)
            o.head1:SetThickness(th); o.head1:SetColorTexture(c[1], c[2], c[3], 1)
            o.head1:SetStartPoint("TOPLEFT", drawLayer, x2, y2)
            o.head1:SetEndPoint("TOPLEFT", drawLayer, b1x, b1y)
            o.head2:SetThickness(th); o.head2:SetColorTexture(c[1], c[2], c[3], 1)
            o.head2:SetStartPoint("TOPLEFT", drawLayer, x2, y2)
            o.head2:SetEndPoint("TOPLEFT", drawLayer, b2x, b2y)
            o.head1:Show(); o.head2:Show()
        else
            o.head1:Hide(); o.head2:Hide()
        end
    end
...
function SlerneNotesViewer.Render()
    ...
    for modName, modData in pairs(SlerneNotesViewer.currentLayout) do
        ...
        if meta.type == "Assignment" then
            -- Create role weights for sorting
            ...
            -- Sort: Role order first, then alphabetical
            ...
            -- Pass 1: configure + measure. The fontstring always sits at LEFT 18
            -- (an icon gutter) so names line up whether or not a row has a role.
            local blockW = 0
            ...
            -- Pass 2: center the name column as a block within the box.
            ...
            if meta.type == "List" or meta.type == "Image List" then
                ...
                for i = 1, meta.length do
                    ...
                    if not row then
                        ...
                        row.label:SetTextColor(1, 1, 1) -- always white, theme-independent
                    ...
                    if playerName then
                        ...
                        if role then
                            ...
                            row.icon:Show()
                            -- FIX: Create a wide 23px gap for the icon
                            row.value:ClearAllPoints()
                            row.value:SetPoint("LEFT", row.label, "RIGHT", 23, 0)
                        else
                            row.icon:Hide()
                            -- FIX: Close the gap down to 5px if there is no icon
                            row.value:ClearAllPoints()
                            row.value:SetPoint("LEFT", row.label, "RIGHT", 5, 0)
                        end
                        
                        -- Wrap only the assigned name (+ role icon), like Slerne Notes
        ...
        elseif meta.type == "Action List" then
            -- 3 read-only columns per row: [player] [middle text] [player]
            -- (mirrors the sender's L<i>/R<i> player keys + labels[i] middle text)
            ...
            -- Fill one player column slot (name, class color, role icon, self-highlight)
        ...
        elseif meta.type == "Text Block" then
            -- No scroll frame: a FontString that sizes the module to its text
            -- (width follows content down to a readable minimum).
    ...
    -- Draw the received strokes + markers on top of the modules
