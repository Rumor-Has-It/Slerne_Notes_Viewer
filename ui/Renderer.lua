...
local localLayer = CreateFrame("Frame", nil, SlerneNotesViewer.canvasPanel)
localLayer:SetAllPoints(SlerneNotesViewer.canvasPanel)
localLayer:SetFrameLevel(SlerneNotesViewer.canvasPanel:GetFrameLevel() + 60)
localLayer:EnableMouse(false)
SlerneNotesViewer.localDrawLayer = localLayer

local DRAW_FONT = select(1, GameFontNormal:GetFont())
local DRAW_CIRCLE_MASK = "Interface\\Masks\\CircleMaskScalable"

local function NewDrawPools()
    return { lines = {}, markers = {}, texts = {}, shapes = {}, lineObjs = {}, joints = {} }
end

local function getJoint(layer, pools, i)
    local t = pools.joints[i]
    if not t then
...
local function RenderDrawingsInto(layer, pools, drawings)
    for _, l in ipairs(pools.lines) do l:Hide() end
    for _, m in ipairs(pools.markers) do m:Hide() end
    for _, t in ipairs(pools.texts) do t:Hide() end
    for _, c in ipairs(pools.shapes) do c:Hide() end
    for _, o in ipairs(pools.lineObjs) do o.main:Hide(); o.head:Hide() end
    for _, j in ipairs(pools.joints) do j:Hide() end
    ...
    for _, stroke in ipairs(drawings.strokes or {}) do
        ...
        local th = stroke.thickness or 3
        local sa = stroke.alpha or 1
        for i = 2, #pts do
            used = used + 1
            local l = pools.lines[used]
            if not l then l = layer:CreateLine(nil, "ARTWORK"); pools.lines[used] = l end
            l:SetThickness(th)
            l:SetColorTexture(c[1], c[2], c[3], sa)
            l:SetStartPoint("TOPLEFT", layer, pts[i - 1][1], pts[i - 1][2])
            l:SetEndPoint("TOPLEFT", layer, pts[i][1], pts[i][2])
            l:Show()
        end
        if th >= 5 then
            for i = 1, #pts do
                jointUsed = jointUsed + 1
                local jt = getJoint(layer, pools, jointUsed)
                jt:SetSize(th, th)
                jt:SetColorTexture(c[1], c[2], c[3], sa)
                jt:ClearAllPoints()
                jt:SetPoint("CENTER", layer, "TOPLEFT", pts[i][1] or 0, pts[i][2] or 0)
                jt:Show()
            end
        end
    end
    ...
    for idx, mk in ipairs(drawings.markers or {}) do
        local m = pools.markers[idx]
        if not m then
            m = CreateFrame("Frame", nil, layer)
        ...
        m:ClearAllPoints()
        m:SetPoint("CENTER", layer, "TOPLEFT", mk.x or 0, mk.y or 0)
        m:Show()
    end

    for idx, sh in ipairs(drawings.shapes or {}) do
        local f = pools.shapes[idx]
        if not f then
        ...
        f:SetAlpha(1)
        f:ClearAllPoints()
        f:SetPoint("CENTER", layer, "TOPLEFT", sh.x or 0, sh.y or 0)
        f:Show()
    end

    for idx, ln in ipairs(drawings.lines or {}) do
        local o = pools.lineObjs[idx]
        if not o then
            o = { main = layer:CreateLine(nil, "ARTWORK"),
                  head = layer:CreateTexture(nil, "ARTWORK") }
            o.head:SetTexture("Interface\\Buttons\\WHITE8x8")
            o.head:SetSize(1, 1)
            o.head:SetPoint("TOPLEFT", layer, "TOPLEFT", 0, 0)
            pools.lineObjs[idx] = o
        end
        ...
        o.main:SetStartPoint("TOPLEFT", layer, x1, y1)
        o.main:SetAlpha(ln.alpha or 1)
        o.head:SetAlpha(ln.alpha or 1)
        o.main:Show()
        if ln.arrow then
            ...
            local px, py = -uy, ux
            o.main:SetEndPoint("TOPLEFT", layer, bx + ux, by + uy)
            ...
            o.main:SetEndPoint("TOPLEFT", layer, x2, y2)
            o.head:Hide()
        end
    end

    for idx, t in ipairs(drawings.texts or {}) do
        local fs = pools.texts[idx]
        if not fs then
            fs = layer:CreateFontString(nil, "OVERLAY")
            fs:SetJustifyH("LEFT")
            pools.texts[idx] = fs
        end
        ...
        fs:SetAlpha(t.alpha or 1)
        fs:ClearAllPoints()
        fs:SetPoint("TOPLEFT", layer, "TOPLEFT", t.x or 0, t.y or 0)
        fs:Show()
    end
end

local mainPools = NewDrawPools()
function SlerneNotesViewer.RenderDrawings(drawings)
    drawLayer:Show()
    RenderDrawingsInto(drawLayer, mainPools, drawings)
end

local localPools = NewDrawPools()
function SlerneNotesViewer.RenderLocalStrokes()
    localLayer:Show()
    local list = SlerneNotesViewer.GetLocalStrokes and SlerneNotesViewer.GetLocalStrokes(false)
    RenderDrawingsInto(localLayer, localPools, { strokes = list or {} })
end
...
local function CollectRegionDrawings(rect)
    local src = SlerneNotesViewer.currentDrawings or {}
    local out = { strokes = {}, markers = {}, texts = {}, shapes = {}, lines = {} }

    local function addStroke(stroke)
        local pts = stroke.points or {}
        local inside = false
        for _, p in ipairs(pts) do
            if rectContainsPoint(rect, p[1] or 0, p[2] or 0) then inside = true; break end
        end
        if not inside then return end
        local shifted = {}
        for i, p in ipairs(pts) do
            shifted[i] = { (p[1] or 0) - rect.x, (p[2] or 0) - rect.y }
        end
        out.strokes[#out.strokes + 1] = {
            color = stroke.color, thickness = stroke.thickness, alpha = stroke.alpha, points = shifted,
        }
    end

    for _, s in ipairs(src.strokes or {}) do addStroke(s) end
    local localList = SlerneNotesViewer.GetLocalStrokes and SlerneNotesViewer.GetLocalStrokes(false)
    for _, s in ipairs(localList or {}) do addStroke(s) end

    for _, mk in ipairs(src.markers or {}) do
        if rectContainsPoint(rect, mk.x or 0, mk.y or 0) then
            out.markers[#out.markers + 1] = {
                kind = mk.kind, icon = mk.icon, size = mk.size, alpha = mk.alpha,
                x = (mk.x or 0) - rect.x, y = (mk.y or 0) - rect.y,
            }
        end
    end
    for _, t in ipairs(src.texts or {}) do
        if rectContainsPoint(rect, t.x or 0, t.y or 0) then
            out.texts[#out.texts + 1] = {
                color = t.color, size = t.size, text = t.text, alpha = t.alpha,
                x = (t.x or 0) - rect.x, y = (t.y or 0) - rect.y,
            }
        end
    end
    for _, sh in ipairs(src.shapes or {}) do
        if rectContainsPoint(rect, sh.x or 0, sh.y or 0) then
            out.shapes[#out.shapes + 1] = {
                color = sh.color, size = sh.size, alpha = sh.alpha,
                x = (sh.x or 0) - rect.x, y = (sh.y or 0) - rect.y,
            }
        end
    end
    for _, ln in ipairs(src.lines or {}) do
        if rectContainsPoint(rect, ln.x1 or 0, ln.y1 or 0) or rectContainsPoint(rect, ln.x2 or 0, ln.y2 or 0) then
            out.lines[#out.lines + 1] = {
                color = ln.color, thickness = ln.thickness, arrow = ln.arrow, alpha = ln.alpha,
                x1 = (ln.x1 or 0) - rect.x, y1 = (ln.y1 or 0) - rect.y,
                x2 = (ln.x2 or 0) - rect.x, y2 = (ln.y2 or 0) - rect.y,
            }
        end
    end
    return out
...
local function RenderModuleContent(modFrame, modName, modData, myName)
...
local popoutFrames = {}
local popoutCascade = 0

local function SpawnModulePopout(modFrame)
    local modName = modFrame._snModName
    local modData = modName and SlerneNotesViewer.currentLayout[modName]
    if not modData then return end

    local pop = popoutFrames[modName]
    if not pop then
        pop = CreateFrame("Frame", nil, UIParent, "BackdropTemplate")
        pop:Hide()
        ...
        pop:SetClipsChildren(true)
        EnsureModuleElements(pop)

        pop.drawLayer = CreateFrame("Frame", nil, pop)
        pop.drawLayer:SetAllPoints(pop)
        pop.drawLayer:SetFrameLevel(pop:GetFrameLevel() + 30)
        pop.drawPools = NewDrawPools()

        pop.closeBtn = CreateFrame("Button", nil, pop, "UIPanelCloseButton")
        pop.closeBtn:SetPoint("TOPRIGHT", -2, -2)
        pop.closeBtn:SetFrameLevel(pop:GetFrameLevel() + 40)
        pop.closeBtn:SetScript("OnClick", function() pop:Hide() end)
        SlerneNotesViewer.Skin.CloseButton(pop.closeBtn)

        pop:SetScript("OnMouseWheel", function(self, delta)
            local s0 = self:GetScale()
            local s1 = math.max(0.4, math.min(2.5, s0 + delta * 0.1))
            if s1 == s0 then return end
            local cx, cy = self:GetCenter()
            self:SetScale(s1)
            self:ClearAllPoints()
            self:SetPoint("CENTER", UIParent, "BOTTOMLEFT", cx * s0 / s1, cy * s0 / s1)
        end)
        popoutFrames[modName] = pop
    end

    local wasShown = pop:IsShown()
    RenderModuleContent(pop, modName, modData, GetMyName())

    local rect = {
        x = (modFrame:GetLeft() or 0) - (drawLayer:GetLeft() or 0),
        y = (modFrame:GetTop() or 0) - (drawLayer:GetTop() or 0),
        w = modFrame:GetWidth() or 0,
        h = modFrame:GetHeight() or 0,
    }
    RenderDrawingsInto(pop.drawLayer, pop.drawPools, CollectRegionDrawings(rect))

    if not wasShown then
        local n = popoutCascade % 6
        popoutCascade = popoutCascade + 1
        pop:ClearAllPoints()
        pop:SetPoint("CENTER", UIParent, "CENTER", n * 30, -n * 30)
    end
    pop:Show()
end
...
function SlerneNotesViewer.Render()
    ...
    for modName, modData in pairs(SlerneNotesViewer.currentLayout) do
        ...
        if not modFrame then
            ...
            EnsureModuleElements(modFrame)

            modFrame.exportBtn = CreateFrame("Button", nil, modFrame, "UIPanelButtonTemplate")
            modFrame.exportBtn:SetSize(18, 18)
            modFrame.exportBtn:SetPoint("TOPRIGHT", -3, -3)
            modFrame.exportBtn:SetText("E")
            modFrame.exportBtn:SetFrameLevel(modFrame:GetFrameLevel() + 10)
            SlerneNotesViewer.Skin.Button(modFrame.exportBtn)
            modFrame.exportBtn:SetScript("OnClick", function() SpawnModulePopout(modFrame) end)
            modFrame.exportBtn:HookScript("OnEnter", function(self)
                GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
                GameTooltip:SetText("Pop out")
                GameTooltip:AddLine("Movable copy on your screen. Mousewheel resizes it.", 0.8, 0.8, 0.8)
                GameTooltip:Show()
            end)
            modFrame.exportBtn:HookScript("OnLeave", function() GameTooltip:Hide() end)

            modPool[index] = modFrame
        end

        modFrame._snModName = modName
        RenderModuleContent(modFrame, modName, modData, myName)
